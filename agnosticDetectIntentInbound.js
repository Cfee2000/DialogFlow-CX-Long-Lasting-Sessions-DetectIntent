// //Join the TaskRouter parameters with any dynamic routing context that was passed from DialogFlow CX
// const mergedJSON = {
//   ...taskRouterParams1,
//   ...data.AgentHandoffParameters
// }
...
exports.handler = async function (context, event, callback) {
  let syncMapKey = "";
  let paramsJSON = {};
  console.log("Channel Source: " + event.ChannelSource);

  if(event.ChannelSource == "SMS" || event.ChannelSource == "Whatsapp" ){
    syncMapKey = event.Phone.substring(event.Phone.lastIndexOf('+') + 1);
    paramsJSON = {
            "fields": {
              "mobile_number" : {
                "kind": "stringValue",
  ...
  else if(event.ChannelSource == "API"){
    syncMapKey = event.email;
        paramsJSON = {
            "fields": {
              "email" : {
                "kind": "stringValue",
    ...
    syncMapKey = event.gbm_agent.substring(event.gbm_agent.lastIndexOf(':') + 1);;
        paramsJSON = {
            "fields": {
              "gbm_agent" : {
                "kind": "stringValue",
    ...
    console.error("Channel Source: " + event.ChannelSource + " does not match SMS or API. Check Studio logs for trigger.conversation.Source and add to Function code in agnostic_detect_intent_inbound.js" );
  }
...
console.log("after response");
    ...
    if (response.queryResult.currentPage.displayName != 'End Session') {
      ...
      await sync.deleteMap(context.SYNC_SID, syncMapKey);
    }
